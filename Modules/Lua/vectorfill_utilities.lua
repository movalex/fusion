...
function vectorfill_utilities.auto_process(tool, comp, options)
    options = options or {}
    ...
    local success = vectorfill_utilities.generate_canvas(tool, comp, {show_progress = show_progress})
    if not success then
        return false
    end

    -- Step 4: Set start frame and process growth
    vectorfill_utilities.set_start_frame(tool, comp)
...
--- @return boolean Success status
function vectorfill_utilities.generate_canvas(tool, comp, options)
    options = options or {}
    local show_progress = options.show_progress ~= false
    ...
    for frame = render_start, render_end do
        ...
        if frame ~= render_start and not flib.file_exists(newName) then
