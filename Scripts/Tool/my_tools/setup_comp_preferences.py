...
def process_metadata(metadata, key_fragment, comp_pref_key, convert_func, value_type, key_string=None):
    if not key_string:
...
def set_from_loader(tool):
    ...
    try:
        ...
        print("To set default pixel aspect ratio, add the Loader to the viewer")
...
def main():
    ...
    fps_found = process_metadata(
        tool_metadata, "fps", "Comp.FrameFormat.Rate", convert_to_number, float, key_string="FrameRate"
    )
    if not fps_found:
