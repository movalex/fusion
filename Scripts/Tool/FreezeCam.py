...
try:
    ...
    if (not tool) or (tool.ID != "Camera3D"):
        raise WrongToolException(
            "You must select a Camera3D tool in the flow to run this script."
        )
    ...
    frozen.SetAttrs(
        {
            "TOOLS_Name": "{}_ProjectFrame{:03}".format(tool.Name, int(comp.CurrentTime)),
            "TOOLB_NameSet": True,
        }
    )
    if ADDCOMMENT:
        frozen.Comments[comp.TIME_UNDEFINED] = "%s @ frame %d" % (
            tool.Name,
            comp.CurrentTime,
        )

    # copy a snapshot of all inputs
    for inp in list(frozen.GetInputList().values()):
