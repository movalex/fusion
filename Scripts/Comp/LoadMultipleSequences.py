# -*- coding: utf-8 -*-
# by ISOTRON, thanks to Helge, https://suedlich-t.com
import os
import re
import random
import fnmatch
...
def rename():
    for i in range(0, len(rename_list)):
        if rename_list[i][0] in longfilename.lower():  # Name ist in Matrix
            ...
            loader.SetAttrs(
                {'TOOLS_Name': str(rename_list[i][1]),
                 'TOOLB_NameSet': True})
            return
...
input = comp.AskUser(
    "Suedlich-t  >>>  Import Some Sequences",
    {
        1.0: {
            1.0: "filepath",
            2.0: "PathBrowse",
        ...
        2.0: {
                1.0: "seqdigits",
                2.0: "Slider",
                'Name': "Sequence has at least so many digits: ",
                'Default': 3,
                'Integer': True,
                'Min': 2,
                'Max': 5},
...
if input:
    ...
    # Dictionary
    sequencedict = {0.0: "Can not do anything"}
    orgnamedict = {}
    ...
    for i in range(1, sequencelength):
        mycode = mycode + str(float(i)) + ': {1.0: "sequence_' + str(
            i) + '", 2.0: "Checkbox", \'Name\':sequencedict[' + str(float(i)) + '], \'Default\':' + preselect + ',}, '
    mycode = mycode + str(float(sequencelength+1)) + ': {1.0: "bla", 2.0: "Text", \'Name\':"' + \
        randomname + '", \'Default\': "' + randomquote + '", \'ReadOnly\': True, \'Wrap\': True,},'
    mycode = mycode+'})'
            ...
            if seqlist['sequence_'+str(i)] == 1:
                ...
                if input['rename'] == 1:
                    ...
                    if longfilename.lower() in rename_list:
                        print(longfilename.lower() + " has it!")
