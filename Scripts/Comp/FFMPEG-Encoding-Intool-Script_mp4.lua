...
-- Step 3. Render a short test sequence in Fusion. You should have a new .mp4 movie and a log .txt file saved in the same folder as your rendered image sequence.
...
if os.getenv('programfiles') == nil then
	...
	-- ffmpegProgramPath = '/opt/KartaVR/linux_tools/ffmpeg/bin/ffmpeg'
...
-- -------------------------------------------------------
command = ffmpegProgramPath .. ' ' .. ffmpegApplyGammaCorrection .. ' -framerate '.. framerate .. ' -f image2 -start_number ' .. comp.RenderStart .. ' -i "' .. ffmpegImageSequenceFilename .. '" -r '.. framerate .. ' -y -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" -f mp4 -vcodec libx264 -pix_fmt yuv420p  -acodec aac -strict -2  "' .. ffmpegMovieFilename .. '" >> "' .. ffmpegLogFilename .. '" 2>&1'
