...
# Once the script finishes, load the .comp files in a text editor, select
# all, copy, and paste into fusion.
...
def AddKeyArray(
    ...
    for Frame in range(FirstFrame, LastFrame + 1):
        # print(Frame)
        KeyArrayString += (
            "     [" + str(Frame) + "] = {" + str(DataArray[Frame - 1]) + "},\n"
        )
    KeyArrayString += "     }\n"
    if IsLast:
        KeyArrayString += "}\n"
...
def IsCurveFlat(
    TheCurve,
):  # checks to see if a curve's has keys but all the keys are the same
    InitialValue = TheKeyValue = mc.keyframe(
        TheCurve, q=True, index=(0, 0), vc=True)
...
def FindAllKeyedAttributes(TheObj, IgnoreFlatCurves):
    ...
    NonKeyedAttributes = []
    AllAttrs = []
    if KeyableAttributes is not None:
        for ThisAttribute in KeyableAttributes:
            try:
                if (mc.keyframe((TheObj + "." + ThisAttribute),
                                q=True, keyframeCount=True)) != 0:
                    if IgnoreFlatCurves:
...
def ExportFusionCamera(
    Cam, TheFileName=""
):  # for each camera, export a .comp file
    print("Exporting Camera :" + Cam)
    if TheFileName == "":
        FileTypes = "Comp Files (*.comp);;All Files (*.*)"
        TheFileName = mc.fileDialog2(
            cap="File name for camera comp file?",
    ...
    FileString += AddKeyArray(
        Shape + "XOffset",
        "Red = 250, Green = 59, Blue = 49",
        PositionXArray,
        FirstFrame,
        LastFrame,
    )  # add position x keys
    ...
    FileString += AddKeyArray(
        Shape + "AngleofView",
        "Red = 8, Green = 229, Blue = 117",
        FOVArray,
        FirstFrame,
        LastFrame,
        IsLast=True,
    )  # add focal length keys
...
def ExportFusionObject(
    Objects, TheFileName=""
):  # for all non-camera selected animated objects export a .comp file
    if TheFileName == "":
        FileTypes = "Comp Files (*.comp);;All Files (*.*)"
        TheFileName = mc.fileDialog2(
            cap="File name for object comp file?",
            fileFilter=FileTypes,
            ds=1,
            fm=0)[0]
    ...
    for Obj in Objects:
        ...
        if IsAnimated(Obj):
            AnimatedCurves = FindAllKeyedAttributes(
                Obj, IgnoreFlatCurves=True)[1]
            ...
            for Frame in range(FirstFrame, LastFrame + 2):
                if "translateX" in AnimatedCurves:
                    PositionXArray.append(
                        Round(
                            mc.getAttr(
                                Obj + ".tx",
                                time=Frame)))
                if "translateY" in AnimatedCurves:
                    PositionYArray.append(
                        Round(
                            mc.getAttr(
                                Obj + ".ty",
                                time=Frame)))
                if "translateZ" in AnimatedCurves:
                    PositionZArray.append(
                        Round(
                            mc.getAttr(
                                Obj + ".tz",
                                time=Frame)))

                if "rotateX" in AnimatedCurves:
                    RotationXArray.append(
                        Round(
                            mc.getAttr(
                                Obj + ".rx",
                                time=Frame)))
                if "rotateY" in AnimatedCurves:
                    RotationYArray.append(
                        Round(
                            mc.getAttr(
                                Obj + ".ry",
                                time=Frame)))
                if "rotateZ" in AnimatedCurves:
                    RotationZArray.append(
                        Round(
                            mc.getAttr(
                                Obj + ".rz",
                                time=Frame)))

                if "scaleX" in AnimatedCurves:
                    ScaleXArray.append(
                        Round(
                            mc.getAttr(
                                Obj + ".sx",
                                time=Frame)))
                if "scaleY" in AnimatedCurves:
                    ScaleYArray.append(
                        Round(
                            mc.getAttr(
                                Obj + ".sy",
                                time=Frame)))
                if "scaleZ" in AnimatedCurves:
                    ScaleZArray.append(
                        Round(
                            mc.getAttr(
                                Obj + ".sz",
            ...
            FileString += "			},\n"
            FileString += (
                "			ViewInfo = OperatorInfo { Pos = { 0, " + str(n * 35) + " } },\n"
            )
            FileString += "	    },\n"
...
def FusionExporter(CameraFileName="", ObjectFileName=""):
    # find the selected objects, sort them into cameras and other animated
    # objects
    ...
    for (
        ...
        SelObjs
    ):  # look through every object selected, find out if its a camera or other animated obj
    ...
    if CameraFileName == "" and ObjectFileName == "":
        if Objects == [] and Cameras == []:
            mc.confirmDialog(
                m="No animated objects or cameras selected! Exiting.",
                button="OK",
                title="Fusion Exporter",
            )
        else:
