...
def _func(ev):
    ...
    if choice <= 1:
        ...
        sx, sy = flow.GetPosTable(source).values() # position of bookmarked node
        # would be nice to store the scale with each bookmark and reference here
        scaleFactor = 1.25 #the actual SetScale value also used to place temp PipeRouters for the hack
        # we have the position of the bookmarked node
        # (if multiple nodes then we could first get average X and Y of all selected -- list, add up, divide by count)
        prRelPosX = round(8 / scaleFactor) # an amount to offset in X - temp PipeRouter from the bookmarked tool
        prRelPosY = round(6 / scaleFactor) # an amount to offset in Y - temp PipeRouter from the bookmarked tool
        pr1 = comp.AddTool("PipeRouter", sx - prRelPosX, sy - prRelPosY)
        pr2 = comp.AddTool("PipeRouter", sx + prRelPosX + 1, sy + prRelPosY + 2) # additional added to favor top left
