...
def build_command(job_details):
    ...
    if not is_movie_format(file_path.name):
        ...
        file_path = os.path.join(
            file_path.parent,
            re.sub("(.*[_.])\d+$", r"\1%0{}d".format(len(padding)), file_path.stem)
            + file_path.suffix,
        )
    ...
    command = (
        f"{ffmpeg_path} {ffmpeg_apply_gamma_correction} "
        f" -i  {file_path} -r {frame_rate} -y "
        " -f mp4 -vcodec libx264 -pix_fmt yuv420p"
        f" -acodec aac {output_filename}"
    )
